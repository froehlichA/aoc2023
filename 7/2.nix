...
in with builtins; with lib; rec {
	...
	countOccurences' = list: let
		original = countOccurences list;
		jNum = if (original ? J) then original.J else 0;
		new = removeAttrs original ["J"];
		keys = attrNames new;
		getAt = k: getAttr k new;
		biggestKey = foldl' (x: y: if ((getAt x) > (getAt y)) then x else y)
			(elemAt keys 0)
			keys;
	in if new == {} then { A = 5; } else (new // (setAttrByPath [biggestKey] ((getAt biggestKey) + jNum)));
	...
	strengthOfCard = card: findIndex card 0 ["J" "2" "3" "4" "5" "6" "7" "8" "9" "T" "Q" "K" "A"];
	cmpCards = a: b: let
