...
input foreach(i, v,
  (v asCharacter == "*") ifTrue(
    ...
    # If symbol is gear, add numbers to map
    (nums size == 2) ifTrue(
      writeln("COMPUTED => ", nums)
      ratio := (nums at(0) number) * (nums at(1) number)
      gear_ratio_sum = gear_ratio_sum + ratio
    )
    writeln("===")
  )
)

gear_ratio_sum println